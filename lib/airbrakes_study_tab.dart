...
class AirBrakesStudyTab extends StatefulWidget {
  final bool resumeFromLast;

  const AirBrakesStudyTab({Key? key, this.resumeFromLast = false})
      : super(key: key);

  @override
  State<AirBrakesStudyTab> createState() => _AirBrakesStudyTabState();
}

class _AirBrakesStudyTabState extends State<AirBrakesStudyTab> {
  final PageController _pageController = PageController();
  int _currentPage = 0;

  int _getCorrectAnswerIdForAirBrakes(int questionId) {
    // Adjust logic if needed, here I kept the same formula
  ...
  Future<void> _saveProgress(int page) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('airbrakes_progress_questionsbank_study', page);
  }
                ...
                Expanded(
                  child: PageView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildQuestionPage(questions, index),
                  ),
                ),
  ...
  Widget _buildQuestionPage(List<dynamic> questions, int index) {
    ...
    final correctId = _getCorrectAnswerIdForAirBrakes(questionId);
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            ...answers
                                .where((ans) => ans["answerId"] == correctId)
                                .map(
                                  (ans) => Container(
